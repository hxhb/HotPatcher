...
FString UFlibHotPatcherCoreHelper::GetCookAssetsSaveDir(const FString& BaseDir, const FString PacakgeName, const FString& Platform)
{
	...
	// UE_LOG(LogHotPatcherCoreHelper,Log,TEXT("Filename:%s,PackageFileName:%s,StandardFileName:%s"),*Filename,*PackageFilename,*StandardFilename);
...
#endif
	{
		...
		for(auto& Platform:CookPlatforms)
		{
			...
			if(bSuccessed)
			{
				if (CookActionCallback.PackageSavedCallback)
				{
					CookActionCallback.PackageSavedCallback(Package,TargetPlatform,ESavePackageResult::Success);
				}
			}
			else
			{
...
void UFlibHotPatcherCoreHelper::CacheForCookedPlatformData(UPackage* Package, TArray<ITargetPlatform*> TargetPlatforms, bool bStorageConcurrent, bool bWaitShaderComplate)
{
	...
	if (bStorageConcurrent)
	{
		for (const ITargetPlatform* TargetPlatform : TargetPlatforms)
		{
			GIsCookerLoadingPackage = true;
			{
				Package->PreSave(TargetPlatform);
			}
			GIsCookerLoadingPackage = false;
		}
	}
