 ...
 */
class GITSOURCECONTROLEX_API FGitScopedTempFile
{
public:
 ...
 */
bool GITSOURCECONTROLEX_API CheckGitAvailability(const FString& InPathToGitBinary, FGitVersion* OutVersion = nullptr);
 ...
 */
 void GITSOURCECONTROLEX_API ParseGitVersion(const FString& InVersionString, FGitVersion* OutVersion);
 ...
 * @param OutGitVersion			If provided, populate with the git version parsed from "version" command
 */
void GITSOURCECONTROLEX_API FindGitCapabilities(const FString& InPathToGitBinary, FGitVersion *OutVersion);
 ...
 * @param OutGitVersion			If provided, populate with the git version parsed from "version" command
 */
 void GITSOURCECONTROLEX_API FindGitLfsCapabilities(const FString& InPathToGitBinary, FGitVersion *OutVersion);
 ...
 * @returns true if the command succeeded and returned no errors
 */
bool GITSOURCECONTROLEX_API RunUpdateStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InFiles, TArray<FString>& OutErrorMessages, TArray<FGitSourceControlState>& OutStates);
 ...
 * @returns true if the command succeeded and returned no errors
*/
bool GITSOURCECONTROLEX_API RunDumpToFile(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InParameter, const FString& InDumpFileName);
 ...
 */
bool GITSOURCECONTROLEX_API RunGetHistory(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InFile, bool bMergeConflict, TArray<FString>& OutErrorMessages, TGitSourceControlHistory& OutHistory);

/**
 * Helper function for various commands to update cached states.
 * @returns true if any states were updated
 */
bool GITSOURCECONTROLEX_API UpdateCachedStates(const TArray<FGitSourceControlState>& InStates);
 ...
 */
void GITSOURCECONTROLEX_API RemoveRedundantErrors(FGitSourceControlCommand& InCommand, const FString& InFilter);
